...
def swine_align(player_score, opponent_score):
    ...
    while player_score % gcd != 0 or opponent_score % gcd != 0:
        gcd -= 1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1, running_high = running_high, last_score = last_score):
        if who == 0:
            diff = score0 - last_score
            if diff > running_high:
                print("{0} point(s)! The most yet for Player {1}".format(diff,who))
                running_high = diff
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    averaged = 0
    while rolls <= 10:
        averaged_roll_dice = make_averaged(roll_dice, trials_count)(rolls, dice)
        if averaged_roll_dice > averaged:
            averaged = averaged_roll_dice
            num_rolls = rolls
        rolls += 1
    return num_rolls
    "*** YOUR CODE HERE ***"
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score+add_score,opponent_score):
        return 0
    elif bacon_strategy(score = score, opponent_score = opponent_score, cutoff = cutoff, num_rolls = num_rolls) == 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
